...
uintptr_t rkp_pagetable_manange_init(u_register_t x1,u_register_t x2,u_register_t x3,u_register_t x4,void *handle){
    ...
    if(sizeof(void*) != sizeof(u_register_t)){
        ...
        ERROR("more info: sizeof 'unsigned long'-0x%lu, sizeof 'unsigned long long'-0x%lu",sizeof(unsigned long),sizeof(unsigned long long));
        goto finished;
    }
    if(x2 != POOLSZIE){
        result = -1;
        ERROR("bad POOLSZIE %lu, not %d",x2,POOLSZIE);
        goto finished;
    }
    PTPOOL = pa_start;
...
uintptr_t rkp_set_pagetable(u_register_t x1,u_register_t x2,u_register_t x3,u_register_t x4,void *handle){
    ...
    *target = content;
    ...
    switch(pagetable_type){
        case 0:
            ERROR("write PTE to 0x%016lx 0x%016lx",x2,x3);
            break;
        case 1:
            ERROR("write PMD to 0x%016lx 0x%016lx",x2,x3);
            break;
        case 2:
            ERROR("write PUG to 0x%016lx 0x%016lx",x2,x3);
            break;
        case 3:
            ERROR("write PGD to 0x%016lx 0x%016lx",x2,x3);
            break;
        default:
            ERROR("write to 0x%016lx 0x%016lx",x2,x3);
            break;        
    }
    SMC_RET1(handle,result);
}
